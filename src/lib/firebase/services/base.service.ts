...
export class BaseService {
  ...
  public create = async <T extends DocumentData>(
    data: WithFieldValue<T>,
  ): Promise<DocumentReference<T>> => {
    try {
      const docRef = await addDoc(collection(firestore, this.dbName), {
        ...data,
        createAt: new Date(),
        updateAt: new Date(),
      });
  ...
  public get = async <T>({ key, opt, value }: IFilterOpt): Promise<T[]> => {
    try {
      const q = query(
        collection(firestore, this.dbName),
        where(key, opt, value),
      );
  ...
  public delete = async (id: string): Promise<void> => {
    try {
      const docRef = doc(firestore, this.dbName, id);
